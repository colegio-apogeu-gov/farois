...
import { Regional, Escola } from '../../types';
...
export function EscolasPage() {
  ...
  const loadData = async () => {
    try {
      const [escolasResponse, regionaisResponse] = await Promise.all([
        supabase
          .from('escolas')
          .select(`
            *,
  ...
  const handleEdit = (escola: Escola) => {
    ...
    setFormData({ 
      nome: escola.nome,
      regional_id: escola.regional_id.toString()
    });
    setShowModal(true);
  };
  ...
  const handleDelete = async (escola: Escola) => {
    ...
    try {
      ...
      toast.error(error.message || 'Erro ao excluir escola');
    }
  };
  ...
  const filteredEscolas = escolas.filter(escola =>
    ...
    escola.regional?.nome.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Buscar por nome da escola ou regional..."
              value={searchTerm}
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          {filteredEscolas.length === 0 ? (
            <EmptyState
              ...
              description={searchTerm ? "Tente ajustar os filtros de busca" : "Cadastre a primeira escola"}
              actionLabel={!searchTerm ? "Nova Escola" : undefined}
                  ...
                  {filteredEscolas.map((escola) => (
                    <tr key={escola.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <p className="font-medium text-gray-900">{escola.nome}</p>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-gray-600">{escola.regional?.nome}</p>
                      </td>
