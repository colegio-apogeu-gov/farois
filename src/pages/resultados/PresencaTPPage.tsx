...
import { Regional, Escola, ResultadoPresencaTP } from '../../types';
import { calculatePresencaCLTFarol } from '../../utils/farol';
import { getQuinzenaLabel, formatNumber } from '../../utils/format';
import toast from 'react-hot-toast';

export function PresencaTPPage() {
  const [resultados, setResultados] = useState<ResultadoPresencaTP[]>([]);
  const [regionais, setRegionais] = useState<Regional[]>([]);
  const [escolas, setEscolas] = useState<Escola[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingResultado, setEditingResultado] = useState<ResultadoPresencaTP | null>(null);
  ...
  const handleEdit = (resultado: ResultadoPresencaTP) => {
  ...
  const handleDelete = async (resultado: ResultadoPresencaTP) => {
  ...
  if (loading) {
    return (
      <ResultadosLayout 
        ...
        description="Registre resultados quinzenais de presença dos técnicos pedagógicos"
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Registros quinzenais de presença dos técnicos pedagógicos</p>
          </div>
          <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingResultado ? 'Editar Resultado' : 'Novo Resultado'} - Presença TP
            </h2>
