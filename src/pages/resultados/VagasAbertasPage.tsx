...
import { Regional, Escola, ResultadoVagasAbertas } from '../../types';
import { calculateVagasAbertasFarol } from '../../utils/farol';
import { getQuinzenaLabel } from '../../utils/format';
import toast from 'react-hot-toast';

export function VagasAbertasPage() {
  const [resultados, setResultados] = useState<ResultadoVagasAbertas[]>([]);
  const [regionais, setRegionais] = useState<Regional[]>([]);
  const [escolas, setEscolas] = useState<Escola[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingResultado, setEditingResultado] = useState<ResultadoVagasAbertas | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.regional_id || !formData.escola_id || !formData.quinzena || 
        !formData.total_vagas || !formData.dias_em_aberto) {
      toast.error('Todos os campos são obrigatórios');
      return;
    }

    const totalVagas = Number(formData.total_vagas);
    const diasEmAberto = Number(formData.dias_em_aberto);

    if (totalVagas < 0 || diasEmAberto < 0) {
      toast.error('Valores não podem ser negativos');
      return;
    }

    if (totalVagas === 0 && diasEmAberto > 0) {
      toast.error('Se não há vagas, os dias em aberto devem ser 0');
      return;
    }
  ...
  const handleEdit = (resultado: ResultadoVagasAbertas) => {
    ...
    setFormData({
      ...
      total_vagas: resultado.total_vagas.toString(),
      dias_em_aberto: resultado.dias_em_aberto.toString()
    });
    loadEscolas(resultado.regional_id);
    setShowModal(true);
  };

  const handleDelete = async (resultado: ResultadoVagasAbertas) => {
  ...
  if (loading) {
    return (
      <ResultadosLayout 
        ...
        description="Registre resultados quinzenais de vagas em aberto"
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Registros quinzenais de vagas em aberto</p>
          </div>
          <button
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredResultados.map((resultado) => {
                    const farol = calculateVagasAbertasFarol(resultado.total_vagas, resultado.dias_em_aberto);
                    return (
                      <tr key={resultado.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <p className="text-gray-900">{resultado.total_vagas}</p>
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-gray-900">{resultado.dias_em_aberto} dias</p>
                        </td>
                        <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingResultado ? 'Editar Resultado' : 'Novo Resultado'} - Vagas em Aberto
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label htmlFor="total_vagas" className="block text-sm font-medium text-gray-700 mb-2">
                  Número total de vagas em aberto *
                </label>
                <input
                  id="total_vagas"
                  type="number"
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, total_vagas: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Ex: 2"
                  required
                />
              ...
              <div>
                <label htmlFor="dias_em_aberto" className="block text-sm font-medium text-gray-700 mb-2">
                  Quantidade de dias que estão em aberto *
                </label>
                <input
                  id="dias_em_aberto"
                  type="number"
                  min="0"
                  value={formData.dias_em_aberto}
                  onChange={(e) => setFormData(prev => ({ ...prev, dias_em_aberto: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Ex: 7"
                  required
                />
