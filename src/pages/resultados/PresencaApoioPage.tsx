...
import { Regional, Escola, ResultadoPresencaApoio } from '../../types';
import { calculatePresencaCLTFarol } from '../../utils/farol';
import { getQuinzenaLabel, formatNumber } from '../../utils/format';
import toast from 'react-hot-toast';

export function PresencaApoioPage() {
  const [resultados, setResultados] = useState<ResultadoPresencaApoio[]>([]);
  const [regionais, setRegionais] = useState<Regional[]>([]);
  const [escolas, setEscolas] = useState<Escola[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingResultado, setEditingResultado] = useState<ResultadoPresencaApoio | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.regional_id || !formData.escola_id || !formData.quinzena || 
        !formData.quantidade_pessoas || !formData.dias_trabalhados || !formData.dias_deveriam) {
      toast.error('Todos os campos são obrigatórios');
      return;
    }

    const quantidadePessoas = Number(formData.quantidade_pessoas);
    const diasTrabalhados = Number(formData.dias_trabalhados);
    const diasDeveriam = Number(formData.dias_deveriam);

    if (quantidadePessoas <= 0 || diasTrabalhados < 0 || diasDeveriam <= 0) {
    ...
    try {
      ...
      setFormData({ 
        ...
        quantidade_pessoas: '', dias_trabalhados: '', dias_deveriam: '' 
      });
      loadData();
    } catch (error: any) {
  ...
  const handleEdit = (resultado: ResultadoPresencaApoio) => {
    ...
    setFormData({
      ...
      quantidade_pessoas: resultado.quantidade_pessoas.toString(),
  ...
  const handleDelete = async (resultado: ResultadoPresencaApoio) => {
  ...
  if (loading) {
    return (
      <ResultadosLayout 
        ...
        description="Registre resultados quinzenais de presença do pessoal de apoio"
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Registros quinzenais de presença do pessoal de apoio</p>
          </div>
          <button
                  ...
                  {filteredResultados.map((resultado) => {
                    ...
                    return (
                      <tr key={resultado.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <p className="text-gray-900">{resultado.quantidade_pessoas}</p>
                        </td>
                        <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingResultado ? 'Editar Resultado' : 'Novo Resultado'} - Presença Apoio
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label htmlFor="quantidade_pessoas" className="block text-sm font-medium text-gray-700 mb-2">
                  Quantas pessoas de apoio/administrativo? *
                </label>
                <input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, quantidade_pessoas: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Ex: 15"
                  required
                />
