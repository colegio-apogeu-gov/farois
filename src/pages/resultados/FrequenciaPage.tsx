...
import { Regional, Escola, ResultadoFrequencia, MetaFrequencia } from '../../types';
import { calculateFrequenciaFarol } from '../../utils/farol';
import { formatNumber, parseDecimalInput } from '../../utils/format';
import toast from 'react-hot-toast';

export function FrequenciaPage() {
  const [resultados, setResultados] = useState<ResultadoFrequencia[]>([]);
  const [metas, setMetas] = useState<MetaFrequencia[]>([]);
  const [regionais, setRegionais] = useState<Regional[]>([]);
  const [escolas, setEscolas] = useState<Escola[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingResultado, setEditingResultado] = useState<ResultadoFrequencia | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.regional_id || !formData.escola_id || !formData.resultado.trim()) {
      toast.error('Todos os campos são obrigatórios');
      return;
    }

    const resultado = parseDecimalInput(formData.resultado);
    if (resultado < 0 || resultado > 100) {
      toast.error('Resultado deve estar entre 0 e 100%');
      return;
    }
    ...
    try {
      ...
      setFormData({ regional_id: '', escola_id: '', resultado: '' });
      loadData();
    } catch (error: any) {
  ...
  const handleEdit = (resultado: ResultadoFrequencia) => {
    setEditingResultado(resultado);
    setFormData({
      regional_id: resultado.regional_id.toString(),
      escola_id: resultado.escola_id.toString(),
      resultado: resultado.resultado.toString().replace('.', ',')
    });
    loadEscolas(resultado.regional_id);
    setShowModal(true);
  };

  const handleDelete = async (resultado: ResultadoFrequencia) => {
  ...
  if (loading) {
    return (
      <ResultadosLayout 
        title="Frequência" 
        description="Registre resultados anuais de frequência"
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Registros anuais de frequência</p>
          </div>
          <button
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredResultados.map((resultado) => {
                    ...
                    const farol = calculateFrequenciaFarol(resultado.resultado, metaAnual);
                    return (
                      <tr key={resultado.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <p className="text-gray-900">{formatNumber(resultado.resultado, 2)}%</p>
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-gray-900">{formatNumber(metaAnual, 2)}%</p>
                        </td>
                        <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingResultado ? 'Editar Resultado' : 'Novo Resultado'} - Frequência
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label htmlFor="resultado" className="block text-sm font-medium text-gray-700 mb-2">
                  Qual foi o valor da frequência? *
                </label>
                <input
                  id="resultado"
                  type="text"
                  ...
                  required
                />
                <p className="text-xs text-gray-500 mt-1">Use vírgula para decimais (ex: 88,50). Percentual de 0 a 100.</p>
              </div>
