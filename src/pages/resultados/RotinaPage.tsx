...
import { Regional, Escola, ResultadoRotina } from '../../types';
import { calculateRotinaFarol } from '../../utils/farol';
import { getQuinzenaLabel, formatNumber } from '../../utils/format';
import toast from 'react-hot-toast';

export function RotinaPage() {
  const [resultados, setResultados] = useState<ResultadoRotina[]>([]);
  const [regionais, setRegionais] = useState<Regional[]>([]);
  const [escolas, setEscolas] = useState<Escola[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingResultado, setEditingResultado] = useState<ResultadoRotina | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.regional_id || !formData.escola_id || !formData.quinzena || 
        !formData.rotinas_cumpridas || !formData.meta_rotinas) {
      toast.error('Todos os campos são obrigatórios');
      return;
    }

    const rotinasCumpridas = Number(formData.rotinas_cumpridas);
    const metaRotinas = Number(formData.meta_rotinas);

    if (rotinasCumpridas < 0 || metaRotinas <= 0) {
      toast.error('Valores devem ser positivos');
      return;
    }

    if (rotinasCumpridas > metaRotinas) {
      toast.error('Rotinas cumpridas não pode ser maior que a meta');
      return;
    }
    ...
    try {
      const data = {
        ...
        ano: selectedYear,
        rotinas_cumpridas: rotinasCumpridas,
        meta_rotinas: metaRotinas
      };
      ...
      setFormData({ 
        ...
        rotinas_cumpridas: '', meta_rotinas: '' 
      });
      loadData();
    } catch (error: any) {
  ...
  const handleEdit = (resultado: ResultadoRotina) => {
    ...
    setFormData({
      ...
      rotinas_cumpridas: resultado.rotinas_cumpridas.toString(),
      meta_rotinas: resultado.meta_rotinas.toString()
    });
    loadEscolas(resultado.regional_id);
    setShowModal(true);
  };

  const handleDelete = async (resultado: ResultadoRotina) => {
  ...
  if (loading) {
    return (
      <ResultadosLayout 
        title="Rotina" 
        description="Registre resultados quinzenais de cumprimento de rotinas"
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Registros quinzenais de cumprimento de rotinas</p>
          </div>
          <button
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredResultados.map((resultado) => {
                    const farol = calculateRotinaFarol(resultado.rotinas_cumpridas, resultado.meta_rotinas);
                    return (
                      <tr key={resultado.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm">
                            ...
                            <p className="text-gray-500">{resultado.rotinas_cumpridas}/{resultado.meta_rotinas} rotinas</p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingResultado ? 'Editar Resultado' : 'Novo Resultado'} - Rotina
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label htmlFor="rotinas_cumpridas" className="block text-sm font-medium text-gray-700 mb-2">
                  Quantas rotinas foram cumpridas? *
                </label>
                <input
                  id="rotinas_cumpridas"
                  type="number"
                  min="0"
                  value={formData.rotinas_cumpridas}
                  onChange={(e) => setFormData(prev => ({ ...prev, rotinas_cumpridas: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Ex: 5"
                  required
                />
              ...
              <div>
                <label htmlFor="meta_rotinas" className="block text-sm font-medium text-gray-700 mb-2">
                  Qual a meta de rotinas para essa quinzena? *
                </label>
                <input
                  id="meta_rotinas"
                  type="number"
                  min="1"
                  value={formData.meta_rotinas}
                  onChange={(e) => setFormData(prev => ({ ...prev, meta_rotinas: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Ex: 7"
                  required
                />
