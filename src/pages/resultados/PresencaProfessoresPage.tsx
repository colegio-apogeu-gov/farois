...
import { Regional, Escola, ResultadoPresencaProfessores } from '../../types';
import { calculatePresencaCLTFarol } from '../../utils/farol';
import { getQuinzenaLabel, formatNumber } from '../../utils/format';
import toast from 'react-hot-toast';

export function PresencaProfessoresPage() {
  const [resultados, setResultados] = useState<ResultadoPresencaProfessores[]>([]);
  const [regionais, setRegionais] = useState<Regional[]>([]);
  const [escolas, setEscolas] = useState<Escola[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingResultado, setEditingResultado] = useState<ResultadoPresencaProfessores | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.regional_id || !formData.escola_id || !formData.quinzena || 
        !formData.quantidade_professores || !formData.dias_trabalhados || !formData.dias_deveriam) {
      toast.error('Todos os campos são obrigatórios');
      return;
    }

    const quantidadeProfessores = Number(formData.quantidade_professores);
    const diasTrabalhados = Number(formData.dias_trabalhados);
    const diasDeveriam = Number(formData.dias_deveriam);

    if (quantidadeProfessores <= 0 || diasTrabalhados < 0 || diasDeveriam <= 0) {
      toast.error('Valores devem ser positivos');
      return;
    }

    if (diasTrabalhados > diasDeveriam) {
      toast.error('Dias trabalhados não pode ser maior que dias que deveriam ser trabalhados');
      return;
    }
    ...
    try {
      const data = {
        ...
        ano: selectedYear,
        quantidade_professores: quantidadeProfessores,
        dias_trabalhados: diasTrabalhados,
        dias_deveriam: diasDeveriam
      };
      ...
      setFormData({ 
        regional_id: '', escola_id: '', quinzena: '', 
        quantidade_professores: '', dias_trabalhados: '', dias_deveriam: '' 
      });
      loadData();
    } catch (error: any) {
  ...
  const handleEdit = (resultado: ResultadoPresencaProfessores) => {
    setEditingResultado(resultado);
    setFormData({
      regional_id: resultado.regional_id.toString(),
      escola_id: resultado.escola_id.toString(),
      quinzena: resultado.quinzena.toString(),
      quantidade_professores: resultado.quantidade_professores.toString(),
      dias_trabalhados: resultado.dias_trabalhados.toString(),
      dias_deveriam: resultado.dias_deveriam.toString()
    });
    loadEscolas(resultado.regional_id);
    setShowModal(true);
  };

  const handleDelete = async (resultado: ResultadoPresencaProfessores) => {
  ...
  if (loading) {
    return (
      <ResultadosLayout 
        ...
        description="Registre resultados quinzenais de presença dos professores"
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Registros quinzenais de presença dos professores</p>
          </div>
          <button
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredResultados.map((resultado) => {
                    const farol = calculatePresencaCLTFarol(resultado.dias_trabalhados, resultado.dias_deveriam);
                    return (
                      <tr key={resultado.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <p className="text-gray-900">{getQuinzenaLabel(resultado.quinzena)}</p>
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-gray-900">{resultado.quantidade_professores}</p>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-sm">
                            <p className="text-gray-900">{formatNumber(farol.value as number, 1)}%</p>
                            <p className="text-gray-500">{resultado.dias_trabalhados}/{resultado.dias_deveriam} dias</p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingResultado ? 'Editar Resultado' : 'Novo Resultado'} - Presença Professores
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label htmlFor="quantidade_professores" className="block text-sm font-medium text-gray-700 mb-2">
                  Quantos professores? *
                </label>
                <input
                  ...
                  type="number"
                  min="1"
                  value={formData.quantidade_professores}
                  onChange={(e) => setFormData(prev => ({ ...prev, quantidade_professores: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Ex: 50"
                  required
                />
              ...
              <div>
                <label htmlFor="dias_trabalhados" className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, dias_trabalhados: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Ex: 480"
                  required
                />
              ...
              <div>
                <label htmlFor="dias_deveriam" className="block text-sm font-medium text-gray-700 mb-2">
                  Quantos dias deveriam ser trabalhados? *
                </label>
                <input
                  id="dias_deveriam"
                  type="number"
                  min="1"
                  value={formData.dias_deveriam}
                  onChange={(e) => setFormData(prev => ({ ...prev, dias_deveriam: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Ex: 500"
                  required
                />
