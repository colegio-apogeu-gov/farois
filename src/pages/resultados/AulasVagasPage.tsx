...
import { Regional, Escola, ResultadoAulasVagas } from '../../types';
import { calculateAulasVagasFarol } from '../../utils/farol';
import { getQuinzenaLabel } from '../../utils/format';
import toast from 'react-hot-toast';

export function AulasVagasPage() {
  const [resultados, setResultados] = useState<ResultadoAulasVagas[]>([]);
  const [regionais, setRegionais] = useState<Regional[]>([]);
  const [escolas, setEscolas] = useState<Escola[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingResultado, setEditingResultado] = useState<ResultadoAulasVagas | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = {
        ...
        ano: selectedYear,
        aulas_vagas: formData.aulas_vagas
      };
      ...
      setFormData({ regional_id: '', escola_id: '', quinzena: '', aulas_vagas: false });
      loadData();
    } catch (error: any) {
  ...
  const handleEdit = (resultado: ResultadoAulasVagas) => {
    ...
    setFormData({
      ...
      aulas_vagas: resultado.aulas_vagas
    });
    loadEscolas(resultado.regional_id);
    setShowModal(true);
  };

  const handleDelete = async (resultado: ResultadoAulasVagas) => {
  ...
  if (loading) {
    return (
      <ResultadosLayout 
        title="Aulas Vagas" 
        description="Registre resultados quinzenais de aulas vagas"
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Registros quinzenais de aulas vagas</p>
          </div>
          <button
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredResultados.map((resultado) => {
                    const farol = calculateAulasVagasFarol(resultado.aulas_vagas);
                    return (
                      <tr key={resultado.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <p className="text-gray-900">
                            {resultado.aulas_vagas ? '1 ou mais aulas vagas' : '0 aulas vagas'}
                          </p>
                        </td>
                        <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingResultado ? 'Editar Resultado' : 'Novo Resultado'} - Aulas Vagas
            </h2>
                ...
                <div className="space-y-2">
                  <label className="flex items-center">
                    <input
                      type="radio"
                      name="aulas_vagas"
                      checked={!formData.aulas_vagas}
                      onChange={() => setFormData(prev => ({ ...prev, aulas_vagas: false }))}
                      className="mr-2"
                    />
                    <span>0 aulas vagas</span>
                  </label>
                  <label className="flex items-center">
                    <input
                      type="radio"
                      name="aulas_vagas"
                      checked={formData.aulas_vagas}
                      onChange={() => setFormData(prev => ({ ...prev, aulas_vagas: true }))}
                      className="mr-2"
                    />
                    <span>1 ou mais aulas vagas</span>
                  </label>
                </div>
              </div>
